...
export const favoriteFetch = createAsyncThunk('favoriteArticleSlice/favoriteFetch', async (slug) => {
  try {
...
export const dislikeFetch = createAsyncThunk('favoriteArticleSlice/dislikeFetch', async (slug) => {
  try {
    const { data } = await axios.delete(`${URL}articles/${slug}/favorite`, {
      headers: {
        Authorization: `Bearer ${getToken()}`,
      },
    })
