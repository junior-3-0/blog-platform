...
import { ControlArticle } from '../ControlArticle'
...
export default function Article({
  ...
  const likeClick = () => {
    ...
    if (!favorit) {
      return like(favoriteFetch)
    }
    return like(dislikeFetch)
  }
      ...
      <div className={cn(styles.flex, { [styles.flex_slug]: params.slug })}>
        <div className={styles.wrap}>
          <div className={styles.inner_wrap}>
            ...
            <button className={cn(styles.like_btn, { [styles.active]: jwt })} type="button" onClick={likeClick}>
              <img src={favorit ? '../path4.svg' : '../heart.svg'} alt="likes" className={styles.likes_img} />
            </button>
          ...
          >
            {tagList &&
              setTags.map((tag) => (
                <button type="button" key={tag} className={styles.tags}>
                  {params.slug ? tag : truncate(tag, 15)}
                </button>
              ))}
          </div>
          <div className={styles.descr}>{params.slug ? description : truncate(description, 220)}</div>
        </div>
        <div className={styles.user_wrap}>
          <div className={styles.user}>
            ...
            <div className={styles.avatar}>
              <img src={author.image} alt="avatar" className={styles.avatar_img} />
            </div>
          </div>
          {username === author.username && jwt && params.slug && <ControlArticle slug={slug} />}
        </div>
      </div>
